...
There is no unique way to predict the agent. Statistical regression models or machine learning algorithms can be applied. In our program, we allow the users to specify their own method, which includes conventional logistic regression, random forest, and XGBoost. The final imputed agent is supported between 0 and 1, which quantifies the disease risk of interest.
